...
class OutputTab(QWidget):
    def __init__(self):
        ...
        self.format_q_sb = QSpinBox()
        self.format_q_sb.setRange(1,100)
        
        self.format_q_sl = QSlider(Qt.Horizontal)
        self.format_q_sl.setRange(1, 100)
        self.format_q_sl.setTickPosition(QSlider.TicksBelow)
        self.format_q_sl.setTickInterval(5)
        self.format_q_sl.valueChanged.connect(self.onQualitySlChange)
        self.format_q_sb.valueChanged.connect(self.onQualitySbChange)
    ...
    def onFormatChange(self):
        ...
        self.format_lossless_cb.setChecked(False)     # It needs to in the beginning
        ...
        if cur_format in ("JPEG XL", "WEBP"):
            self.format_lossless_cb.setEnabled(True)
            self.format_lossless_if_cb.setEnabled(True)
        else:
            self.format_lossless_cb.setEnabled(False)
        ...
        # Effort / Speed
        if cur_format in ("JPEG XL", "AVIF"):
            self.format_e_sb.setEnabled(True)
        else:
            self.format_e_sb.setEnabled(False)

        # Disable Quality Slider
        if cur_format in ("PNG", "Smallest Lossless"):
            self.format_q_sb.setEnabled(False)
            self.format_q_sl.setEnabled(False)
        else:
            self.format_q_sb.setEnabled(True)
            self.format_q_sl.setEnabled(True)
        
        # Quality Slider Range and label
        if cur_format == "AVIF":
            self.format_q_sl.setRange(-63, 0)
            self.format_q_sb.setRange(0, 63)
            self.format_e_sb.setRange(0, 10)
            self.format_e_sb.setValue(6)
            self.format_q_sl.setValue(-20)
    ...
    def onQualitySlChange(self):
        self.format_q_sb.setValue(abs(self.format_q_sl.value()))

    def onQualitySbChange(self):
        if self.format_cmb.currentText() == "AVIF":
            self.format_q_sl.setValue(-self.format_q_sb.value())
        else:
            self.format_q_sl.setValue(self.format_q_sb.value())

    def toggleEffort(self):
        if self.format_e_sb.isEnabled():
            self.format_e_sb.setEnabled(False)
        else:
            self.format_e_sb.setEnabled(True)
    ...
    def setEffortVisible(self, visible):
        ...
        self.format_e_sb.setVisible(visible)
        self.format_e_l.setVisible(visible)
    ...
    def setLosslessVisible(self, visible):
        ...
        self.format_lossless_cb.setVisible(visible)
        self.format_lossless_if_cb.setVisible(visible)